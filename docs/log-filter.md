...
1. **URL Encoding**: When using in browser/API clients, make sure to URL-encode the JSON string
2. **Nested Access**: Use dot notation for nested fields: `userData.userId`, `moduleData.moduleId`
3. **Type Matching**: Operators respect types (e.g., `$gt` only works with numbers)
4. **Implicit AND**: Multiple top-level conditions are automatically combined with AND
5. **Shorthand Equality**: `{"level": "error"}` is equivalent to `{"level": {"$eq": "error"}}`

## Implementation Details

- **Phase 1** (Implemented): `$eq`, `$ne`, `$in`, `$nin`, `$gt`, `$gte`, `$lt`, `$lte`, `$exists`, `$regex`, nested field access
- **Phase 2** (Implemented): `$and`, `$or` logical operators
- Filter is applied during stream processing for memory efficiency
- Invalid filter JSON returns a 400 Bad Request error
